...
class DataProcessing:
    def __init__(self, data, target_column, random_state_val, alpha, precompute,
                 ...
                 fit_intercept, copy_X, max_iter, positive, shuffle, l1_ratio):
        ...
        self.l1_ratio = l1_ratio # float in [0, 1]
    ...
    def train_linear_regression_lasso_and_ridge(self):
      
        self.linear_model_lasso_and_ridge = ElasticNet(
        ...
        self.linear_model_lasso_and_ridge.fit(self.X_train, self.y_train)

    # Так называемый расчет метрик качества регресии
    def evaluate_coefficients_and_mse(self):
        coefficients = self.linear_model_lasso_and_ridge.coef_
        mse_train = mean_squared_error(y_true=self.y_train, y_pred=self.linear_model_lasso_and_ridge.predict(self.X_train))
        mse_test = mean_squared_error(y_true=self.y_test, y_pred=self.linear_model_lasso_and_ridge.predict(self.X_test))
        r_squared_train = r2_score(y_true=self.y_train, y_pred=self.linear_model_lasso_and_ridge.predict(self.X_train))
        r_squared_test = r2_score(y_true=self.y_test, y_pred=self.linear_model_lasso_and_ridge.predict(self.X_test))
