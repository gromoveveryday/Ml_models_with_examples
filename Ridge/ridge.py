...
class DataProcessing:
    def __init__(self, data, target_column, random_state_val, alpha,
                 tol, solver, random_state_ridge, test_size,
        ...
        self.tol = tol # float
        self.solver = solver # «auto», «svd», «cholesky», «lsqr», «sparse_cg», «sag», «saga» 
        ...
        self.random_state_ridge = random_state_ridge # int (Ridge)
    ...
    def train_linear_regression_ridge(self):
      
        self.linear_model_ridge = Ridge(
            ...
            solver=self.solver, # алгоритмы решения
            positive=self.positive, # убирает неположительные коэффициенты из модели
            random_state=self.random_state_ridge, # начальное значение генератора случайных числе
             )
        
        self.linear_model_ridge.fit(self.X_train, self.y_train)

    # Так называемый расчет метрик качества регресии
    def evaluate_coefficients_and_mse(self):
        coefficients = self.linear_model_ridge.coef_
        mse_train = mean_squared_error(y_true=self.y_train, y_pred=self.linear_model_ridge.predict(self.X_train))
        mse_test = mean_squared_error(y_true=self.y_test, y_pred=self.linear_model_ridge.predict(self.X_test))
        r_squared_train = r2_score(y_true=self.y_train, y_pred=self.linear_model_ridge.predict(self.X_train))
        r_squared_test = r2_score(y_true=self.y_test, y_pred=self.linear_model_ridge.predict(self.X_test))
